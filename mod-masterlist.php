...
$sth = $db->prepare("SELECT orient FROM members");
$sth->execute();
...
<div class="subheadline">Vaccine List</div>
 ...
 <?php
  try {
    $query = "SELECT username, UserHex, status, StarveDate FROM members WHERE status='vaccine' OR status='UsedVaccine' ORDER BY username";
...
<div class="subheadline">Gamer List</div>
 ...
 <?php
  try {
    $query = "SELECT username, UserHex, status, KillCount, StarveDate FROM members WHERE status='human' OR status='zombie' OR status='deceased' ORDER BY StarveDate DESC";
