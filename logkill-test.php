...
// CHECKS IF HEX MATCHES SOMETHING IN DATABASE AND RETURNS THE VICTIM'S USERNAME
...
function regKill($cxn, $victim)
{
    ...
    {
        $query_updateKill = "UPDATE members SET status='zombie' WHERE username='$victim'"; // CHANGE BACK TO 'ZOMBIE' AFTER TESTING
...
if (isset($_POST['check_list'])) 
{
  ...
  $arr = array('evan1','jackieisgreat', '4f83d7e0e', 'bb3f89edf1', '282e8918e41', '6b21d842f3e', 'aa0bba8');


    // UPDATES STARVE DATES
    // Takes victim's and zombie's usernames
    ...
    function updateStarveImmune($cxn, $zombieFeedto, $zombieFeeder)
    {
        date_default_timezone_set('America/Denver');
...
if(strcmp($victim, "none") == 0)
{
  ...
  if (in_array($hex, $arr)) {
    ...
    echo "success kill <br>";
    $arr = array_diff($arr, array($hex));
    print_r($arr);

    /* $starveUpdate = updateStarveImmune($cxn, $zombieFeedto, $zombieFeeder);
    if($starveUpdate)
    {
        echo "success kill";
    } */
